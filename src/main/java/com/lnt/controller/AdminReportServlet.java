...
public class AdminReportServlet extends HttpServlet {
    ...
    private String getQueryForType(String type) {
        return switch (type) {
            case "candidates" -> "SELECT candidate_id, full_name, email, phone, education, skills, active, created_at FROM candidates ORDER BY candidate_id";
            case "employers" -> "SELECT employer_id, company_name, email, industry, website, active, created_at FROM employers ORDER BY employer_id";
            case "jobs" -> "SELECT job_id, employer_id, title, location, salary, skills_required, experience_required, deadline, active, created_at FROM jobs ORDER BY job_id";
            case "applications" -> "SELECT application_id, candidate_id, job_id, status, applied_at FROM applications ORDER BY application_id";
            default -> null;
        };
    }
